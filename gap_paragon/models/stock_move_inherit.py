...
class PartnerRiskExceededWizInherit(models.TransientModel):
    ...
    def _compute_mostrar_boton(self):
        for record in self:
            ...
            record.mostrar_boton = self._context.get('active_model') == 'sale.order' or self.env.ref('account_financial_risk.group_overpass_partner_risk_exception').id in group_ids